...
def test_backend_audit_has_no_legacy_inbound_adapters():
    backend = inventory()["backend"]
    unregistered = {
        item["file"]
    ...
    assert all("_compat.py" not in item for item in unregistered)
...
def test_runtime_composition_uses_canonical_bpm_operational_adapter():
    ...
    operational = [item for item in runtime if item["path"].startswith("/api/bpm/operational/")]
    assert operational
    assert all("_compat" not in item["module"] for item in operational)


def test_runtime_composition_uses_canonical_tree_and_analysis_adapters():
    runtime = inventory()["backend"]["runtime"]["routes"]
    tree = [item for item in runtime if item["path"].startswith("/api/rca-tree/")]
    analysis = [item for item in runtime if item["path"].startswith("/api/rca-tree/analys")]
    assert tree and analysis
    assert all("_compat" not in item["module"] for item in tree + analysis)


def test_analysis_consumers_use_rca_tree_persistence():
    from pathlib import Path

    for relative in (
        "uc_bib_solv/modules/rca_tree/adapters/outbound/analysis_postgres.py",
        "uc_bib_solv/modules/rca_tree/application/use_cases/analyses/create_analysis.py",
    ):
...
def test_canonical_runtime_rejects_legacy_routes():
    ...
    assert not any(path.startswith(("/api/operational/", "/api/process-modeling/", "/api/causas", "/api/hipotesis/", "/api/analyses", "/api/analysis-templates")) for path, _ in runtime_paths)
