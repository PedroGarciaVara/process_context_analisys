...
const machines = fs.readFileSync(new URL("../../uc_bib_solv/webapp/js/views/maquinas/maquinas.js", import.meta.url), "utf8");
const contracts = fs.readFileSync(new URL("../../uc_bib_solv/webapp/js/views/bpm/contratos.js", import.meta.url), "utf8");
...
test("contratos delega opciones al componente compartido y no renderiza status", () => {
  ...
  assert.doesNotMatch(contracts, /statusBadge|contractStatus|data-contract-filter|Estado/);
  assert.doesNotMatch(contractUi, /statusBadge/);
});
