...
class ProcessModelingApiTests(unittest.TestCase):
    ...
    def test_list_uses_stable_envelope(self, _list):
        response = self.client.get("/api/bpm/processes")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.get_json(), {"status": "ok", "data": [{"process_code": "P-1"}]})

    def test_process_endpoint_is_canonical(self):
        response = self.client.get(f"/api/bpm/processes/{uuid4()}")
        self.assertIn(response.status_code, (404, 409))
        self.assertNotIn("Traceback", response.get_data(as_text=True))

    @patch("uc_bib_solv.modules.platform.infrastructure.app_factory.process_modeling_service.validate_process", return_value={"valid": True, "errors": []})
    @patch("uc_bib_solv.modules.platform.infrastructure.app_factory.process_modeling_service.get_process", return_value={"process_id": "should-not-be-returned"})
    def test_validate_endpoint_runs_validation_use_case(self, get_process, validate_process):
        process_id = str(uuid4())
        response = self.client.post(f"/api/bpm/processes/{process_id}/validate")
        self.assertEqual(response.status_code, 200)
    ...
    @patch(
        "uc_bib_solv.modules.platform.infrastructure.app_factory.process_modeling_service.get_process",
    ...
    def test_get_process_forwards_expansion_node(self, get_process):
        parent_id = str(uuid4())
        node_id = str(uuid4())
        response = self.client.get(f"/api/bpm/processes/{parent_id}?expand_node_id={node_id}")
        self.assertEqual(response.status_code, 200)
    ...
    @patch(
        "uc_bib_solv.modules.platform.infrastructure.app_factory.process_modeling_service.create_node_with_transition",
    ...
    def test_atomic_node_endpoint_forwards_payload(self, create_node_with_transition):
        ...
        response = self.client.post(f"/api/bpm/processes/{process_id}/nodes-with-transition", json=payload)
