...
class Amd002DomainTests(unittest.TestCase):
    def test_stock_24_is_structured_and_output_roles_are_explicit(self):
        ...
        normal = ProcessNode(process_id=process_id, node_code="NORMAL", node_type="output", name="Normal", output_role="normal")
        waste = ProcessNode(process_id=process_id, node_code="WASTE", node_type="output", name="Waste", output_role="waste")
    ...
    def test_decision_requires_yes_no_branch_targets(self):
        process_id = str(uuid4())
        decision = ProcessNode(process_id=process_id, node_code="DEC", node_type="decision", name="Decisión")
        normal = ProcessNode(process_id=process_id, node_code="OK", node_type="output", name="OK", output_role="normal")
        waste = ProcessNode(process_id=process_id, node_code="NOK", node_type="output", name="NOK", output_role="waste")
        yes = ProcessTransition(process_id=process_id, source_node_id=decision.node_id, target_node_id=normal.node_id, transition_type="branch", label="Sí")
        no = ProcessTransition(process_id=process_id, source_node_id=decision.node_id, target_node_id=waste.node_id, transition_type="branch", label="No")
        self.assertTrue(validate_graph([decision, normal, waste], [yes, no])["valid"])

    def test_rejects_incomplete_decision_branches(self):
        process_id = str(uuid4())
        decision = ProcessNode(process_id=process_id, node_code="DEC", node_type="decision", name="Decisión")
        output = ProcessNode(process_id=process_id, node_code="OUT", node_type="output", name="Salida", output_role="normal")
        branch = ProcessTransition(process_id=process_id, source_node_id=decision.node_id, target_node_id=output.node_id, transition_type="branch", label="Sí")
