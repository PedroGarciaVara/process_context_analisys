...
from uc_bib_solv.modules.rca_tree.domain.causal_graph.operations import ensure_acyclic, project_tree
...
class CausalTreeDomainT6Tests(unittest.TestCase):
    ...
    def test_cycle_rule_is_framework_free(self):
        ...
        with self.assertRaises(CycleDetectedError):
            ensure_acyclic(edges, 2, 1)

    def test_projection_is_deterministic_and_marks_reuse(self):
        result = project_tree([1, 2], {1: {"id": 1, "name": "A"}, 2: {"id": 2, "name": "B"}, 3: {"id": 3, "name": "C"}}, {1: [3], 2: [3]})
