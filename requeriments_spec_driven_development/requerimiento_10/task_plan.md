...
- **NC-006** (`implementation`, `in_correction`, 2026-08-30): en una captura BPM el nodo de decisión y sus ramas Sí/No se renderizan en serie/vertical, incumpliendo la composición de dos ramas visualmente paralelas y claramente separadas exigida por FR-12/AC-25. T17/T18/T21 cubren lanes independientes, ramas paralelas y separación. Reentrada: `execute-agent`; corrección técnica aplicada en `layout.js` y `tests/unit/process-modeling-layout.test.mjs` (profundidad común y orden Sí→No); requiere nueva validación `validate-implementation`/Gate 3 humano; no se marca `resolved`.
...
- [ ] NC-006 permanece `in_correction`; las ramas Sí/No deben renderizarse en lanes paralelos y separados; pendiente corrección por `execute-agent` y validación humana Gate 3.
