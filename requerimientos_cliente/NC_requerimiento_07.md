indings
  ...
  1. High: el DDL no arranca sobre una base vacía porque las tablas hijas de hipótesis referencian node(id) antes de que node exista. db/schema.sql:65
     crea hypothesis_required_data y hypothesis_expected_evidence con FK a node, pero node no se crea hasta db/schema.sql:113. Esto no lo detectan
  ...
  2. High: la nueva relación CAUSE -> CONTRACT no se renderiza realmente en el árbol consumido por la app. La consulta recursiva ya puede recorrer nodos
     ...
     cuyo node_type no sea CAUSE, y el payload Java recompone el árbol desde esa lista filtrada en uc_bib_solv/webapp_java/python-backend/repositories/
  ...
  3. High: el corte a modelo canónico no está completado; siguen activos caminos funcionales basados en columnas legacy. app/persistence/
     ...
     webapp_java/python-backend/services/causa_detail_service.py:30. Eso deja el comportamiento dependiendo de datos duplicados y puede producir
