...
MACHINE_STATUSES = frozenset({"ready", "running", "stopped", "degraded", "unavailable", "unknown"})
...
JSON_OBJECT_OR_LIST_FIELDS = frozenset({"nominal_capacity", "specific_characteristics", "specific_limitations", "differences_from_machine_type"})
...
def validate_machine_payload(payload: dict, *, partial: bool = False) -> dict:
    ...
    if status is not None and status not in MACHINE_STATUSES:
        raise MachineModelError("operational_status no permitido", "invalid_operational_status")
