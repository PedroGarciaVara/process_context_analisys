...
from ..application.ports.process_modeling import BpmProcessModelingApplication
...
class BpmOperationalService:
    ...
    def list_operations(self, version_id=None): return self.use_cases.list_operations(version_id)
    ...
    def list_machines(self, process_id=None, contract_id=None, operation_id=None, process_version_id=None, bpm_process_id=None):
        return self.use_cases.list_machines(process_id, contract_id, operation_id, process_version_id, bpm_process_id)
    ...
    def get_machine_context(self, machine_id, operation_id=None, process_version_id=None):
        return self.use_cases.get_machine_context(machine_id, operation_id, process_version_id)
    ...
    def get_operational_catalog(self, version_id=None): return self.use_cases.get_catalog(version_id)
    def get_operational_page_payload(self, page, params=None): return self.use_cases.get_page(page, params)
    def get_catalog(self, version_id=None): return self.use_cases.get_catalog(version_id)
...
def build_bpm_operational_service(*, persistence=None):
    return BpmOperationalService(BpmOperationalApplication(persistence or build_bpm_operational_postgres_adapter()))


def build_bpm_process_modeling_application(handler):
    """Inject the transitional handler behind the BPM application port."""
    return BpmProcessModelingApplication(handler)
...
def build_bpm_postgres_persistence_adapter(*, connection_factory=None):
    ...
    from uc_bib_solv.modules.bpm.adapters.outbound.postgres.pm_process_repo import NodeRepository, ProcessRepository, TransitionRepository, VersionRepository
...
def build_operational_service(*, persistence=None):
    return OperationalService(BpmOperationalApplication(persistence or build_bpm_operational_postgres_adapter()))
...
def get_operational_catalog(version_id=None): return operational_service().get_catalog(version_id)
