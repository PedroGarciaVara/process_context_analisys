...
class UpdateProcessNode:
    ...
    def execute(self, node_id, data):
        ...
        entity = ProcessNode(**{key: merged[key] for key in ("node_id", "process_id", "node_code", "node_type", "name", "description", "child_process_id", "output_role", "properties")})
        ...
        process_nodes = self.dependencies.process(node["process_id"]).get("nodes", [])
        if any(item.get("node_id") != node_id and item.get("node_code") == entity.node_code for item in process_nodes):
            raise ProcessModelingError("El código de nodo ya existe en el proceso", "duplicate_node_code")
