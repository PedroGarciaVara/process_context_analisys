                  ...
                  <tr class="hover:bg-surface-container-lowest transition-colors ${activeMachine && activeMachine.id === item.id ? "bg-secondary-container/20 border-l-4 border-l-primary" : ""}" data-machine-row="${escapeHtml(item.id)}">
                    ...
                    <td class="px-lg py-md">
                      <div class="flex justify-end gap-sm">
                        ...
                        <button type="button" class="px-md py-sm border border-red-200 text-red-700 text-label-md font-label-md rounded hover:bg-red-50" data-action="machine-delete" data-machine-id="${escapeHtml(item.id)}">Eliminar</button>
                      </div>
                    </td>
                  </tr>
                `).join("") : `
...
export function renderMaquinas(state, bus) {
  const root = createElement("div", {
    className: "app-page-shell bg-surface font-body-md text-on-surface overflow-hidden h-screen flex flex-col w-full",
  });
  ...
  return {
    ...
    afterMount(mountRoot, currentState, eventBus) {
      ...
      mountRoot.querySelectorAll("[data-action='machine-delete']").forEach((node) => {
        node.addEventListener("click", async (event) => {
          event.preventDefault();
          const machineId = node.getAttribute("data-machine-id");
          const machine = machineId ? findMachine(AppState, machineId) : null;
          const machineName = machine ? catalogName(machine) : "";
          if (!machine || !window.confirm(`¿Eliminar la máquina \"${machineName}\"? Esta acción no se puede deshacer.`)) return;
          node.disabled = true;
          try {
            await deleteMachine(machine.id);
            if (String(AppState.currentMachine || "") === String(machine.id)) setCurrentMachine(null);
            await refreshCatalog();
          } catch (error) {
            node.disabled = false;
            window.alert(error.message || "No se pudo eliminar la máquina.");
          }
        });
      });
