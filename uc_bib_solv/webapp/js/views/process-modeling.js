...
function navigateToSelectedNodeDetail(node) {
  if (node.node_type === "subprocess" && node.child_process_id) {
    window.location.hash = `#/procesos_detalle_v02?bpm_process_id=${encodeURIComponent(node.child_process_id)}`;
    return;
  }
  if (node.node_type === "operation") {
    window.location.hash = `#/operaciones_detalle_v02?process_id=${encodeURIComponent(modelProcessId())}&node_id=${encodeURIComponent(node.node_id)}`;
    return;
  }
