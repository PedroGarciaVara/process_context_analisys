...
import { buildMachineMultiOptions, buildContractScopeOptions, buildContractProcessFilterOptions } from "../../components/contract-ui.js";
      ...
      <section class="space-y-sm">
        ...
        <div class="flex items-end justify-between gap-lg flex-wrap">
          ...
          <div class="flex items-center gap-md flex-wrap justify-end">
            <button type="button" class="px-lg py-md bg-primary text-on-primary rounded-lg font-label-md shadow-sm hover:opacity-90" data-action="contract-create-open" aria-haspopup="dialog">
              <span class="material-symbols-outlined align-middle mr-xs">add</span>Crear contrato
            </button>
          </div>
        </div>
      </section>
      ...
      <section class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden shadow-sm">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-outline-variant">
            ...
            `).join("") : `
              <tr><td colspan="4" class="px-lg py-xl text-center text-on-surface-variant">No hay contratos para el filtro actual.</td></tr>
            `}
          </tbody>
        </table>
      </section>
    ...
    afterMount(mountRoot, currentState, eventBus) {
      ...
      mountRoot.querySelectorAll("[data-action='contract-tree']").forEach((node) => {
        node.addEventListener("click", (event) => {
          ...
          setCurrentContract(contractId);
          if (eventBus) eventBus.emit("state:change");
          window.location.hash = toHashRoute("arboles");
        });
      });
