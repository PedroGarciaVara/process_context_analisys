...
const MAIN_MENU = [
  ...
  { route: "maquinas", label: "Maquina", icon: "settings" },
  { route: "procesos", label: "Proceso", icon: "history" },
...
function buildAnalysisCard(analysis) {
  ...
  const route = `#/analisis_causas?contract_id=${encodeURIComponent(String(analysis.contrato_id))}&analysis_id=${encodeURIComponent(String(analysis.id))}`;
      ...
      <main class="flex-1 overflow-y-auto bg-surface p-xl">
        <div class="w-full max-w-[1600px] mx-auto space-y-xl">
          ...
          <section class="space-y-md">
            ...
            <div class="border-2 border-dashed border-outline-variant rounded-xl flex flex-col items-center justify-center p-lg group cursor-pointer hover:border-primary transition-colors" data-route="analisis_causas" data-action="new-investigation">
    ...
    afterMount(mountRoot, currentState, eventBus) {
      ...
      routeLinks.forEach((item) => {
        item.addEventListener("click", (event) => {
          ...
          if (action === "open-tree") {
            event.preventDefault();
            bindRouteNavigation(item, "arboles", eventBus);
            return;
          }

          if (action === "view-processes") {
            event.preventDefault();
            bindRouteNavigation(item, "procesos", eventBus);
            return;
          }
          ...
          if (action === "new-investigation") {
            event.preventDefault();
            window.location.hash = "#/analisis_causas";
            return;
          }

          if (action === "recent-analysis") {
            event.preventDefault();
            window.location.hash = item.getAttribute("data-analysis-url") || "#/analisis_causas";
            return;
          }
