...
from app.domain.graph import validate_no_cycle, validate_relationship_signature
...
def _default_contract(contract_id: int | None = None) -> dict | None:
    if contract_id:
        ...
        if contract:
            return contract
    contracts = contrato_repo.get_all()
...
def create_contract_child(
    ...
    *,
    ...
    parent_contract = contrato_repo.get_by_id(int(parent_contract_id))
    if not parent_contract:
        raise ValueError("El contrato base no existe.")

    created_contract = contrato_repo.create(
