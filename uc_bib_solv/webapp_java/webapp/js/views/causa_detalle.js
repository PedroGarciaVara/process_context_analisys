...
export function renderCausaDetalle() {
  ...
  setSelectOptions(refs.hypothesisStatus, [
  ...
  function renderDetail(payload) {
    ...
    const causeForm = payload.cause_form || {};
    ...
    refs.hypothesisStatus.value = hypothesisForm.estado || "pendiente";
    ...
    [refs.hypothesisDescription, refs.hypothesisType, refs.hypothesisStatus, refs.hypothesisCriterion,
      ...
      node.disabled = !canEditHypotheses;
    });
  }
  ...
  async function handleSaveCause() {
    if (!state.detail?.contract_id && !state.detail?.causa_id) {
    ...
    try {
      ...
      if (isLinkMode(state.editorMode)) {
        ...
        window.location.hash = buildTreeHash(state.detail?.contract_id);
        return;
      }
      ...
      if (!payload.causa_id && response?.cause?.id) {
        window.location.hash = buildCausaDetalleHash({
  ...
  async function handleSaveHypothesis() {
    ...
    try {
      const payload = {
        ...
        estado: refs.hypothesisStatus.value,
      };
  ...
  function clearHypothesisForm() {
    ...
    refs.hypothesisStatus.value = "pendiente";
  ...
  function editHypothesis(hypothesisId) {
    ...
    refs.hypothesisStatus.value = selected.estado || "pendiente";
  ...
  refs.searchModal.addEventListener("keydown", (event) => {
    if (event.key === "Escape") {
      event.preventDefault();
      closeSearchModal();
    }
