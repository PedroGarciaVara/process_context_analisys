import { readStatus } from "./tree-data.js";
...
function renderTreeCard(node, selectedCauseId, hypothesesByCause, mode, payload) {
  ...
  const title = createElement("h4", "acv2-tree-card-title", node.nombre || `Causa ${causeId}`);
...
export function renderDetailPanel(payload, handlers = {}) {
  ...
  title.textContent = cause.nombre || `Causa ${cause.id}`;
...
export function renderSidebar(payload) {
  ...
  const title = createElement("div", "acv2-sidebar-title", translateTreeLabel(payload.sidebar?.title, "Proyecto Alpha"));
  const subtitle = createElement("div", "acv2-sidebar-subtitle", translateTreeLabel(payload.sidebar?.subtitle, "Vista de arbol"));
  ...
  contractLabel.textContent = translateTreeLabel(payload.sidebar?.contract_label, "Contrato");
  ...
  if (contractOptions.length === 0) {
    ...
    option.textContent = `${payload.contract?.id || "—"} - ${payload.contract?.nombre || "Seleccionado"}`;
...
function renderTopContext(payload) {
  ...
  const title = createElement("div", "acv2-context-title", translateTreeLabel(payload.top_context?.title, "Vista de arbol"));
  const subtitle = createElement("div", "acv2-context-subtitle", translateTreeLabel(payload.top_context?.subtitle, ""));
